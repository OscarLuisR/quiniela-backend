...
export async function getClasificacionByFaseGrupoService(
    idFase: string,
    idGrupo: string,
): Promise<{ docs: IClasificacion[] }> {
    const results = await ClasificacionModel.find({
        idFase: idFase,
        idGrupo: idGrupo,
    })
        .sort({ ranking: 1 })
        .populate({
            path: "idSeleccion",
            model: SeleccionModel,
            select: "_id pais codigo_iso bandera_url",
        });
